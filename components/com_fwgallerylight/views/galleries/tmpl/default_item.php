...
$link = JRoute::_('index.php?option=com_fwgallerylight&view=gallery&id='.$this->row->id.':'.JFilterOutput::stringURLSafe($this->row->name));
...
<div class="col-sm-6 col-md-<?php echo $this->columns; ?> galleries-item isotope-item">
	<div class="fwg-loading galleries-item-image<?php if ($this->row->_videos) { ?> galleries-item-video<?php } ?>">
		<a href="<?php echo $link; ?>" class="img-eye-hover">
			<img src="<?php echo JRoute::_('index.php?option=com_fwgallerylight&view=image&layout=image&format=raw&id='.$this->row->_file_id.'&w=370&h=256'); ?>" alt="<?php echo JFHelper :: escape($this->row->name); ?>"/>
			<figcaption>
<?php
...
?>
		<p><?php echo strip_tags($this->row->descr, 'p'); ?></p>
<?php
}
?>
	</div>
