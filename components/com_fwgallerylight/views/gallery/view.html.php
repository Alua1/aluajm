...
class fwGallerylightViewGallery extends JViewLegacy {
    function display($tmpl=null) {
        ...
        default :
            ...
            if (!$this->obj->id) {
            	$app->redirect(
            		JRoute :: _('index.php?option=com_fwgallerylight&view=galleries&Itemid='.JFHelper :: getItemid('galleries'), false)
        		);
...
/*    		if (!$active_menu_item or ($active_menu_item and strpos($active_menu_item->link, 'galleries') === false ) and $this->params->get('id') != $this->obj->id) {
    	        $pathway->addItem(JText::_('FWG_GALLERIES'), JRoute::_('index.php?option=com_fwgallerylight&view=galleries'));
    		}*/
            ...
            if ($this->path and $this->params->get('id') != $this->obj->id){
            	 foreach ($this->path as $item) {
            	 	$pathway->addItem($item->name, JRoute::_('index.php?option=com_fwgallerylight&view=gallery&id='.$item->id.':'.JFilterOutput :: stringURLSafe($item->name).'&Itemid='.JFHelper :: getItemid('gallery', $item->id, $input->getInt('Itemid'))).'#fwgallerytop');
            	 }
            }
