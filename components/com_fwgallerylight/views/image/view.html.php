...
class fwGallerylightViewImage extends JViewLegacy {
    function display($tmpl=null) {
        ...
        if ($this->params->get('id') != $this->row->id) {
        	if ($path = $model->loadCategoriesPath($this->row->project_id)) foreach ($path as $item) {
		        $pathway->addItem(
		        	$item->name,
		        	JRoute::_('index.php?option=com_fwgallerylight&view=gallery&id='.$item->id.':'.JFilterOutput :: stringURLSafe($item->name).
    ...
    function download() {
        ...
        if ($image->id and JFHelper :: isFileExists($image, 'orig')) {
        	...
        	$app = JFactory :: getApplication();
        	$app->redirect(JRoute :: _('index.php?option=com_fwgallerylight&view=galleries', false), JText :: _('FWG_IMAGE_NOT_FOUND'));
        }
    }
}
